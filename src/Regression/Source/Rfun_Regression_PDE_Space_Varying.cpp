...
#include "../../Skeletons/Include/MixedEffects_Skeleton_time.h"
...
extern "C"
{
        ...
        */
        SEXP regression_PDE_space_varying(SEXP Rlocations, SEXP RbaryLocations, SEXP Robservations, SEXP Rmesh, SEXP Rorder, SEXP Rmydim, SEXP Rndim,
                ...
                SEXP Rsearch, SEXP Roptim, SEXP Rlambda, SEXP Rnrealizations, SEXP Rseed, SEXP RDOF_matrix, SEXP Rtune, SEXP Rsct, SEXP Rweights)
        {
                //Set data
        	RegressionDataEllipticSpaceVarying regressionData(Rlocations, RbaryLocations, Robservations, Rorder, RK, Rbeta, Rc, Ru, Rcovariates, RBCIndices, RBCValues, RincidenceMatrix, RarealDataAvg, Rsearch, Rweights);
	...
	*/
	...
	/*!
		...
		\return R-vectors containg the coefficients of the solution, prediction of the values, optimization data and much more
	*/
          SEXP MixedEffects_PDE_space_varying(SEXP Rlocations, SEXP RbaryLocations, SEXP Robservations, SEXP Rmesh, SEXP Rorder,SEXP Rmydim, SEXP Rndim,
          					SEXP RK, SEXP Rbeta, SEXP Rc, SEXP Ru, SEXP Rcovariates, SEXP RBCIndices, SEXP RBCValues, SEXP RincidenceMatrix, SEXP RarealDataAvg,
                				...
                				SEXP Rrandom_effects_covariates, SEXP Rgroup_ids, SEXP Rn_groups)
        {
        	...
        	UInt ndim = INTEGER(Rndim)[0];
                ...
                if(regressionData.getOrder()==1 && mydim==2 && ndim==2)
                	return(MixedEffects_skeleton<MixedEffectsDataEllipticSpaceVarying, 1, 2, 2>(regressionData, optimizationData, Rmesh));
                else if(regressionData.getOrder()==2 && mydim==2 && ndim==2)
                	return(MixedEffects_skeleton<MixedEffectsDataEllipticSpaceVarying, 2, 2, 2>(regressionData, optimizationData, Rmesh));
                // else if(regressionData.getOrder()==1 && mydim==2 && ndim==3)
                //     return(MixedEffects_skeleton<MixedEffectsDataEllipticSpaceVarying, 1, 2, 3>(regressionData, optimizationData, Rmesh));
                // else if(regressionData.getOrder()==2 && mydim==2 && ndim==3)
                //     return(MixedEffects_skeleton<MixedEffectsDataEllipticSpaceVarying, 2, 2, 3>(regressionData, optimizationData, Rmesh));
                else if(regressionData.getOrder()==1 && mydim==3 && ndim==3)
                    return(MixedEffects_skeleton<MixedEffectsDataEllipticSpaceVarying, 1, 3, 3>(regressionData, optimizationData, Rmesh));
                else if(regressionData.getOrder()==2 && mydim==3 && ndim==3)
                    return(MixedEffects_skeleton<MixedEffectsDataEllipticSpaceVarying, 2, 3, 3>(regressionData, optimizationData, Rmesh));

                return(R_NilValue);
        }
        
        //! This function manages the various options for Mixed Effects Spatio-Temporal Regression
	/*!
		This function is then called from R code.
		\param Rlocations an R-matrix containing the spatial locations of the observations
		\param Rtime_locations an R-vector containing the temporal locations of the observations
		...
		\return R-vectors containg the coefficients of the solution, prediction of the values, optimization data and much more
	*/
        SEXP MixedEffects_PDE_space_varying_time(SEXP Rlocations, SEXP RbaryLocations, SEXP Rtime_locations, SEXP Robservations, SEXP Rmesh, 
        	SEXP Rmesh_time, SEXP Rorder, SEXP Rmydim, SEXP Rndim, SEXP RK, SEXP Rbeta, SEXP Rc, SEXP Ru, SEXP Rcovariates, SEXP RBCIndices,
        	SEXP RBCValues,  SEXP RincidenceMatrix, SEXP RarealDataAvg, SEXP Rflag_mass, SEXP Rflag_parabolic,SEXP Rflag_iterative, 
        	SEXP Rmax_num_iteration, SEXP Rthreshold, SEXP Ric, SEXP Rmax_num_iteration_pirls, SEXP Rthreshold_pirls, 
        	SEXP Rsearch, SEXP Roptim, SEXP Rlambda_S, SEXP Rlambda_T, SEXP Rnrealizations, SEXP Rseed, 
        	SEXP RDOF_matrix, SEXP Rtune, SEXP Rsct,
        	SEXP Rrandom_effects_covariates, SEXP Rgroup_ids, SEXP Rn_groups)
        {
                //Set data
                MixedEffectsDataEllipticSpaceVarying regressionData(Rlocations, RbaryLocations, Rtime_locations, Robservations, Rorder, RK, Rbeta, Rc, Ru,
                         				Rcovariates, RBCIndices, RBCValues, RincidenceMatrix, RarealDataAvg, Rflag_mass, Rflag_parabolic, Rflag_iterative,
                         				Rmax_num_iteration, Rthreshold, Ric, Rsearch, Rmax_num_iteration_pirls, Rthreshold_pirls,
                         				Rrandom_effects_covariates, Rgroup_ids, Rn_groups);
                OptimizationData optimizationData(Roptim, Rlambda_S, Rlambda_T, Rflag_parabolic, Rnrealizations, Rseed, RDOF_matrix, Rtune, Rsct);
                ...
                if(regressionData.getOrder() == 1 && mydim == 2 && ndim==2)
            	         return(MixedEffects_skeleton_time<MixedEffectsDataEllipticSpaceVarying, 1, 2, 2>(regressionData, optimizationData, Rmesh, Rmesh_time));
                else if(regressionData.getOrder() == 2 && mydim == 2 && ndim==2)
                        return(MixedEffects_skeleton_time<MixedEffectsDataEllipticSpaceVarying, 2, 2, 2>(regressionData, optimizationData, Rmesh, Rmesh_time));
                else if(regressionData.getOrder() == 1 && mydim == 3 && ndim==3)
                        return(MixedEffects_skeleton_time<MixedEffectsDataEllipticSpaceVarying, 1, 3, 3>(regressionData, optimizationData, Rmesh, Rmesh_time));
                else if(regressionData.getOrder() == 2 && mydim == 3 && ndim==3)
                        return(MixedEffects_skeleton_time<MixedEffectsDataEllipticSpaceVarying, 2, 3, 3>(regressionData, optimizationData, Rmesh, Rmesh_time));
