...
extern "C"
{
	...
	*/
	...
	/*!
		...
		\return R-vectors containg the coefficients of the solution, prediction of the values, optimization data and much more
	*/
	SEXP MixedEffects_Laplace_time(SEXP Rlocations, SEXP RbaryLocations, SEXP Rtime_locations, SEXP Robservations, SEXP Rmesh, SEXP Rmesh_time, 
		SEXP Rorder, SEXP Rmydim, SEXP Rndim, SEXP Rcovariates, SEXP RBCIndices, SEXP RBCValues,  SEXP RincidenceMatrix, 
		SEXP RarealDataAvg, SEXP Rflag_mass, SEXP Rflag_parabolic,SEXP Rflag_iterative, SEXP Rmax_num_iteration, SEXP Rthreshold, 
		SEXP Ric, SEXP Rmax_num_iteration_pirls, SEXP Rthreshold_pirls, SEXP Rsearch,
		SEXP Roptim, SEXP Rlambda_S, SEXP Rlambda_T, SEXP Rnrealizations, SEXP Rseed, SEXP RDOF_matrix, SEXP Rtune, SEXP Rsct,
		SEXP Rrandom_effects_covariates, SEXP Rgroup_ids, SEXP Rn_groups)
	{
	    	//Set input data
		MixedEffectsDataLaplace regressionData(Rlocations, RbaryLocations, Rtime_locations, Robservations, Rorder, Rcovariates, RBCIndices, RBCValues,
            						RincidenceMatrix, RarealDataAvg, Rflag_mass, Rflag_parabolic,
    							Rflag_iterative, Rmax_num_iteration, Rthreshold, Ric, Rsearch,
            						Rmax_num_iteration_pirls, Rthreshold_pirls,
							Rrandom_effects_covariates, Rgroup_ids, Rn_groups);
		...
		if(regressionData.getOrder()==1 && mydim==2 && ndim==2)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 1, 2, 2>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==2 && mydim==2 && ndim==2)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 2, 2, 2>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==1 && mydim==2 && ndim==3)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 1, 2, 3>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==2 && mydim==2 && ndim==3)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 2, 2, 3>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==1 && mydim==3 && ndim==3)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 1, 3, 3>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==2 && mydim==3 && ndim==3)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 2, 3, 3>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==1 && mydim==1 && ndim==2)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 1, 1, 2>(regressionData, optimizationData, Rmesh, Rmesh_time));
		else if(regressionData.getOrder()==2 && mydim==1 && ndim==2)
			return(MixedEffects_skeleton_time<MixedEffectsDataLaplace, 2, 1, 2>(regressionData, optimizationData, Rmesh, Rmesh_time));	
