...
SEXP GAM_skeleton(InputHandler & GAMData, OptimizationData & optimizationData, SEXP Rmesh, SEXP Rmu0, std::string family, SEXP RscaleParam)
{
  MeshHandler<ORDER, mydim, ndim> mesh(Rmesh, GAMData.getSearch());
	...
	// Factory:
	std::unique_ptr<FPIRLS_GAM<InputHandler, ORDER, mydim, ndim>> fpirls = FPIRLSfactory<InputHandler, ORDER, mydim, ndim>::createFPIRLSsolver(family, mesh, GAMData, optimizationData, mu0, scale_parameter);
